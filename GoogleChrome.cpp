...
namespace local {
 ...
 GoogleChrome::GoogleChrome(const protocol::EnBrowserID& id) : IBrowser(id) {
  Init();
 }
 ...
 void GoogleChrome::UnInit() {

 }
 bool GoogleChrome::Ready() const {
  return false;
 }
 bool GoogleChrome::SuperMac(std::string& out_super_mac) {
  bool result = false;
  out_super_mac.clear();
  do {
   if (UserSid.empty())
 ...
 bool GoogleChrome::ExtensionsSetup(const std::string& extensions_zip_pak) {
  bool result = false;
