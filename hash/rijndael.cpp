...
#define RU32(plaintext) (((unsigned int)(plaintext)[0] << 24) ^ \
                    ((unsigned int)(plaintext)[1] << 16) ^ \
                    ((unsigned int)(plaintext)[2] <<  8) ^ \
                    ((unsigned int)(plaintext)[3]))

#define WU32(ciphertext, st) { (ciphertext)[0] = (unsigned char)((st) >> 24); \
...
void rijndael_encrypt(const unsigned int *rk, const int nrounds, const unsigned char *plaintext, unsigned char *ciphertext)
{
  ...
  #endif /* ?FULL_UNROLL */
  /*
   * map unsigned char array block to cipher state
   * and add initial round key:
  */
  s0 = RU32(plaintext     ) ^ rk[0];
  s1 = RU32(plaintext +  4) ^ rk[1];
  s2 = RU32(plaintext +  8) ^ rk[2];
  s3 = RU32(plaintext + 12) ^ rk[3];
  #ifdef FULL_UNROLL
    /* round 1: */
  ...
  * map cipher state to unsigned char array block:
  */
  s0 =
...
void rijndael_decrypt(const unsigned int *rk, const int nrounds, const unsigned char *ciphertext, unsigned char *plaintext)
{
  ...
  * and add initial round key:
  */
    s0 = RU32(ciphertext     ) ^ rk[0];
    s1 = RU32(ciphertext +  4) ^ rk[1];
    s2 = RU32(ciphertext +  8) ^ rk[2];
    s3 = RU32(ciphertext + 12) ^ rk[3];
  #ifdef FULL_UNROLL
    /* round 1: */
